...
async def test_create_project_with_all_detailed_fields_success(client: AsyncClient) -> None:
    ...
    payload = {
        ...
        "revenue_model": "Freemium: gratuito para usuários, premium para empresas",
        "competitive_advantage": "Algoritmo de otimização de rotas + gamificação + parcerias",
        ...
        "academy_info": "Mestrado em Gestão Ambiental pela USP, Bacharel em Eng.",
        "market_info": "Experiência de 5 anos em desenvolvimento de apps",
    }
...
async def test_create_project_field_aggregation(client: AsyncClient) -> None:
    ...
    quando enviados apenas os campos detalhados.
    """
    student = await create_test_user()
    token_headers = await generate_user_auth_headers(client, student)
