...
export const useProjectForm = (totalSteps: number) => {
  const [currentStep, setCurrentStep] = useState(1);
  const [formData, setFormData] = useState<ProjectFormData>(INITIAL_PROJECT_STATE);
  ...
  const FieldError = useCallback(({ fieldKey }: { fieldKey: keyof ProjectFormData }) => (
    <div className="my-0.2">
      {errors[fieldKey] && <span className="text-sm text-red-500">{errors[fieldKey]}</span>}
    </div>
  ), [errors]);

  const getFieldProps = useCallback((key: keyof ProjectFormData, placeholder: string, isTextArea: boolean = false, minHeightClass?: string) => ({
    value: formData[key],
    placeholder,
    className: `${errors[key] ? "ring-2 ring-red-500 border-red-500" : ""} ${isTextArea && minHeightClass ? minHeightClass : ''}`,
    onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => handleInputChange(key, e.target.value),
  }), [formData, errors, handleInputChange]);

  const stepTitles = useMemo(() => ([
    { title: "Informações Básicas do Projeto", description: "Comece nos contando sobre o seu projeto: título, descrição e a solução que você propõe." },
    { title: "Problema e Proposta de Valor", description: "Defina o problema que você está resolvendo, quem é seu público-alvo e qual valor você entrega." },
    { title: "Modelo de Negócio (Lean Canvas)", description: "Descreva seu segmento de clientes, modelo de receita e vantagem competitiva." },
    { title: "Inovação, Impacto e Viabilidade", description: "Conte sobre a inovação, o impacto social, a viabilidade técnica e o potencial de escalabilidade." },
    { title: "Sobre Você", description: "Queremos conhecer você! Conte sobre sua trajetória, formação acadêmica e experiências." },
  ]), []);
